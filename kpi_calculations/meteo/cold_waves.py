...
class ColdWaves(MeteoBase):
    ...
    def calculate(self):
        ...
        for key,values in self.data["result"].items():
            ...
            group_df['isAlarm'] = group_df['temperature'].rolling(window=3, min_periods=1).max() < config['weather_downscaling']['temperature']['cold_wave']
...
# gross_floor_area = ColdWaves(kpi_name="cold_wave")
