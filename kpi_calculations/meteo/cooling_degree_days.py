...
class CoolingDegreeDays(MeteoBase):
        ...
        for year in years:
            ...
            for station in tqdm(prediction_df['weatherStation'].unique(), desc="Processing Stations"):
                ...
                station_group_df = station_group_df.with_columns(
                    pl.when(pl.col("temperature") - pl.lit(config['weather_downscaling']['temperature']['degree_days']['cooling']) > 0)
                    .then(pl.col("temperature") - pl.lit(config['weather_downscaling']['temperature']['degree_days']['cooling']))
                    .otherwise(0)
                    .alias("CDD")
                )
    ...
    return [
        {
            ...
            "kpis": {row["reference"]: round(row["CDD_sum"], 2) for row in group.iter_rows(named=True)}
        }
