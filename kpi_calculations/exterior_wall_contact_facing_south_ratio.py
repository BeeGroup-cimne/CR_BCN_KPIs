...
class ExteriorWallContactFacingSouthRatio(KPIBase):
    ...
    def calculate(self):
        ...
        for ref, value in self.data["air_contact"].items():
            if isinstance(value, dict):
                exterior_90_270 = sum(v for k, v in value.items() if 90 <= int(k) <= 270)
                total = sum(list(value.values())) + self.data["adiabatic"][ref]
                if total != 0:
                    self.data["result"][ref] = (exterior_90_270 / total) * 100
